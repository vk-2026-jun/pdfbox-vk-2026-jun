...
/**
 * Processor for glyph layout using AWT/HarfBuzz capabilities
 * <p>
 * Use an object of this class only in one thread.
 * Multiple instances can be used in different threads.
 *
...
public class GlyphLayoutProcessorAwt implements GlyphLayoutProcessorInterface {
    ...
    /**
     * Adjustment threshold: only apply positioning adjustments >= one ULP of font size.
     * This prevents noise from floating-point rounding errors.
     */
    private static final float ADJUSTMENT_THRESHOLD_FACTOR = 1.0f;
    ...
    public static void checkMissingGlyphs(String text, Font awtFont) {
        ...
        Objects.requireNonNull(awtFont, "Font must not be null");
     ...
     * <p>
     * This class supports OpenType fonts with description of glyphs as TrueType outlines, i.e. *.ttf-files.
     * *.otf-files using CFF outlines are not supported by PDFBox.
     *
    ...
    /**
     * Computes glyph positioning using AWT layout engine
     *
     * @param font     to be used
     * @param fontSize font size
     * @param text text to show
     * @param bidiLevel as computed by Bidi class, even LTR, odd RTL
     * @return GlyphVector with positioned glyphs
     */
    protected GlyphVector computeGlyphVector(PDType0Font font, float fontSize, String text, int bidiLevel) {
        ...
        // use fractional metrics for precise glyph positioning

        int localFlags = bidiLevel % 2 == 0 ? java.awt.Font.LAYOUT_LEFT_TO_RIGHT : java.awt.Font.LAYOUT_RIGHT_TO_LEFT;

        java.awt.Font awtFont = glyphLayoutFontLoaderAwt.getAwtFont(font).deriveFont(fontSize);
     ...
     * @throws IllegalArgumentException if glyphs are missing or parameters invalid
     */
    ...
    /**
     * Shows a text using glyph positioning (if needed)
     * This text must have a uniform run direction.
     *
     * @param contentStream the content stream
     * @param font     to be used
     * @param fontSize font size
     * @param text text to show
     * @param bidiLevel bidi level (even=LTR, odd=RTL)
    ...
    protected void showTextUni(ContentStreamForGlyphLayoutInterface contentStream, PDType0Font font, float fontSize, String text, int bidiLevel) throws IOException {
        ...
        // Adjustment threshold: only apply adjustments >= one ULP of the font size
        // This prevents noise from floating-point rounding errors
        final float delta = Math.ulp(fontSize) * ADJUSTMENT_THRESHOLD_FACTOR;
